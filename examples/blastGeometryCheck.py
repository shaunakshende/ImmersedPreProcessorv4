...
G0 = ms.generate_unif_PDforeground(O, np.array([1.2, 1.2, 0.32]), np.array([12, 12, 10]), 0)

#G0 = ms.generate_unif_PDforeground(O, np.array([1.2, 1.2, 0.32]), np.array([12, 12, 3]), 0)
G0 = sf.translate(G0, padding_x, padding_x, padding_z)

# # Charge - Material 1 - TNT charge
G1 = ms.generate_FGCone_cylindrical2(O, 0.103/2.0, 0.0085, 0.001, 0.075, [4, 20, 5], 1, 2, 1)
G1 = sf.translate(G1, 1.2, 1.2, 0.32+padding_z)#+h/4.0)

# # Material 2 : CompB (mix of RDX and TNT)
G2 = ms.generate_FGCone_cylindrical2(O, 0.113/2.0, 0.009325, 0.001, 0.08228, [5, 22, 5], 1, 2, 2)
