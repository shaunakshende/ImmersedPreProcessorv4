...
def uniform_refine_1D(region_start, region_end, num_div, refinement_factor):
    ...
    for i in range(num_div):
        X = np.vstack([X, x[i]])
        if(x[i] >= region_start and x[i] < region_end):
...
def background_with_uniform_refinement(C1, C2, t, num_div_vec, xyz, region_x, region_y, region_z, refinement_factor, n=1):
    ...
    #region must be specified in parametric coordinates
    #Refinement factor will scale the cell size in the region of interest as 1/refinement_factor ; default argument n = 1 polynomial order 2 surface is returned. Returns NURBS object stretched mesh.
    O=C1.points[0,:]
    S=ruled(C1, C2)
    S.elevate(1,n)
    S.elevate(0,n)
    if t>0:
        S=extrude(S, t, 2)
        S.elevate(2,n)
    xhat = uniform_refine_1D(region_x[0], region_x[1], num_div_vec[0], refinement_factor)
    yhat = uniform_refine_1D(region_y[0], region_y[1], num_div_vec[1], refinement_factor)
    if(num_div_vec[2]>1):
